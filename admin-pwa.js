/* Sadik Travels Admin Console PWA bootstrap.
 *
 * - Registers the admin-scoped service worker (/admin/sw.js).
 * - Shows a branded, accessible install popup automatically (driven by
 *   `beforeinstallprompt`) so staff get the "Install Admin Console" prompt
 *   without hunting for a button; a 14-day dismissal cooldown prevents nagging.
 * - On iOS Safari (no beforeinstallprompt) shows platform instructions.
 * - Explicit "Install app" buttons ([data-pwa-install], e.g. the sidebar link)
 *   always work regardless of the cooldown.
 */
(() => {
  'use strict';
  const DISMISS_KEY = 'st_admin_pwa_dismissed_at';
  const DISMISS_COOLDOWN_MS = 14 * 24 * 60 * 60 * 1000;
  const AUTO_PROMPT_DELAY_MS = 8000;
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('/admin/sw.js').catch(() => undefined);
    });
  }
  ...
  const popupHtml = (mode) => `
    ...
    <div class="pwa-popup" role="dialog" aria-modal="true" aria-labelledby="pwaPopupTitle">
      ...
      <img class="pwa-popup-logo" src="/assets/admin-icon-192.png" alt="" width="56" height="56" />
      <h2 id="pwaPopupTitle">Install Admin Console</h2>
      <p>Add the operations console to your device for one-tap access to bookings, catalogue and support.</p>
      <ul class="pwa-popup-benefits">
        <li>✓ One-tap access from your home screen</li>
        <li>✓ Booking &amp; support notifications</li>
  ...
  window.addEventListener('appinstalled', () => {
    ...
    if (window.showToast) window.showToast('Admin console installed. Find it on your home screen.', 'success');
  });

  // Explicit install buttons (sidebar) always work regardless of cooldown.
  document.addEventListener('click', (event) => {
    const trigger = event.target.closest('[data-pwa-install]');
    if (!trigger) return;
    event.preventDefault();
    if (isStandalone()) { if (window.showToast) window.showToast('The console is already installed.', 'success'); return; }
