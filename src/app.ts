...
import { optionalAuth, requireAuth, requireAdmin, requirePermission, requireFinePermission, requireSuperAdmin, permissionsFor, notFound, requestContext } from './middleware.js';
...
const contentTypeSchema = z.enum(['homepage', 'destination', 'hotel', 'home', 'visa', 'esim', 'offer', 'airline', 'banner', 'faq', 'company', 'umrah_fare', 'umrah_package', 'holiday_package', 'medical_tourism', 'card_offer', 'airline_offer', 'explore', 'app']);
...
const campaignInputSchema = z.object({ name:z.string().trim().min(2).max(160), title:z.string().trim().max(160).optional(), description:z.string().trim().max(5000).optional(), imageUrl:z.string().url().or(z.literal('')).optional(), imagePublicId:z.string().max(300).optional(), imageMediaId:z.string().uuid().nullable().optional(), cta:z.string().max(100).optional(), ctaUrl:z.string().url().or(z.literal('')).optional(), startDate:z.string().datetime().optional(), endDate:z.string().datetime().optional(), channels:z.array(z.enum(['email','sms','in_app'])).min(1), subject:z.string().max(200).optional(), preheader:z.string().max(300).optional(), senderName:z.string().max(160).optional(), senderEmail:z.string().email().or(z.literal('')).optional(), emailHtml:z.string().max(100000).optional(), emailText:z.string().max(10000).optional(), smsMessage:z.string().max(1000).optional(), webTitle:z.string().max(160).optional(), webMessage:z.string().max(4000).optional(), webCtaText:z.string().max(100).optional(), webCtaUrl:z.string().url().or(z.literal('')).optional(), segmentId:z.string().uuid().nullable().optional(), recipientFilter:z.record(z.unknown()).default({}), scheduledAt:z.string().datetime().optional(), timezone:z.string().max(80).optional(), status:z.enum(['draft','scheduled','archived']).default('draft') });
const campaignPatchSchema = campaignInputSchema.partial();
const campaignTestSchema = z.object({ channel:z.enum(['email','sms','in_app']), destination:z.string().min(3).max(240).optional() });
...
export function buildApp() {
  ...
  const adminRoleSchema = z.enum(['admin', 'staff', 'manager', 'support', 'content_manager', 'finance', 'super_admin']);
  ...
  app.get('/api/v1/admin/navigation', requireAdmin(store), async (_req, res) => res.json({ navigation: await store.listNavigation(false) }));
  app.post('/api/v1/admin/navigation', requirePermission(store, 'navigation:manage'), async (req, res) => { const input=toInput(navigationInputSchema,req.body); const item=await store.createNavigation(input); await store.audit('admin.navigation_created',{...clientMeta(req),userId:req.user!.id,metadata:{navigationId:item.id,label:item.label}}); res.status(201).json({ navigation:item }); });
  ...
  app.patch('/api/v1/admin/users/:id/role', requirePermission(store, 'users:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(roleRequest, req.body); const target = await store.findUserById(String(req.params.id)); assert(target, 404, 'USER_NOT_FOUND', 'User not found'); assert(target.id !== req.user!.id || input.role === 'super_admin', 409, 'SELF_ROLE_CHANGE_BLOCKED', 'You cannot remove your own admin access'); assert(req.user!.role === 'super_admin' || input.role !== 'super_admin', 403, 'SUPER_ADMIN_REQUIRED', 'Only a super admin can grant super-admin access'); const user = await store.setUserRole(target.id, input.role); await store.audit('admin.user_role_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { targetUserId: target.id, previousRole: target.role, role: input.role } }); res.json({ user: userView(user, true) }); });
  ...
  app.get('/api/v1/admin/customers/segments', requirePermission(store, 'customers:view'), async (_req,res)=>res.json({segments:await store.listCustomerSegments()}));
  app.post('/api/v1/admin/customers/segments', requirePermission(store, 'customers:view'), async (req,res)=>{const input=toInput(segmentInputSchema,req.body);const item=await store.createCustomerSegment({...input,createdBy:req.user!.id});res.status(201).json({segment:item,estimatedRecipients:await store.estimateSegment(item.rules)});});
  app.patch('/api/v1/admin/customers/segments/:id', requirePermission(store, 'customers:view'), async (req,res)=>{const input=toInput(segmentInputSchema.partial(),req.body);const item=await store.updateCustomerSegment(String(req.params.id),input);assert(item,404,'SEGMENT_NOT_FOUND','Segment not found');res.json({segment:item,estimatedRecipients:await store.estimateSegment(item.rules)});});
  app.post('/api/v1/admin/customers/segments/estimate', requirePermission(store, 'customers:view'), async (req,res)=>{const rules=z.record(z.unknown()).parse(req.body||{});res.json({estimatedRecipients:await store.estimateSegment(rules)});});
  ...
  app.get('/api/v1/admin/campaigns', requirePermission(store, 'notifications:send'), async (req,res)=>res.json(await store.listCampaigns({q:req.query.q?String(req.query.q):undefined,status:['draft','scheduled','queued','processing','completed','partially_failed','failed','cancelled','archived'].includes(String(req.query.status))?String(req.query.status) as any:'all',page:Number(req.query.page)||1,pageSize:Number(req.query.pageSize)||20})));
  app.post('/api/v1/admin/campaigns', requirePermission(store, 'notifications:send'), async (req,res)=>{const input=toInput(campaignInputSchema,req.body);assert(!input.startDate || !input.endDate || new Date(input.startDate) <= new Date(input.endDate),400,'INVALID_CAMPAIGN_DATES','Campaign end date must be after its start date');const campaign=await store.createCampaign({...input,emailHtml:sanitizeEmailHtml(input.emailHtml),createdBy:req.user!.id});await store.audit('admin.campaign_created',{...clientMeta(req),userId:req.user!.id,metadata:{campaignId:campaign.id,channels:campaign.channels}});res.status(201).json({campaign});});
  app.get('/api/v1/admin/campaigns/templates', requirePermission(store, 'notifications:send'), async (req,res)=>res.json({templates:await store.listCampaignTemplates(req.query.type==='email'||req.query.type==='sms'||req.query.type==='in_app'?req.query.type:undefined)}));
  app.post('/api/v1/admin/campaigns/:id/duplicate', requirePermission(store, 'notifications:send'), async (req,res)=>{const original=await store.findCampaign(String(req.params.id));assert(original,404,'CAMPAIGN_NOT_FOUND','Campaign not found');const {id:_id,createdAt:_createdAt,updatedAt:_updatedAt,totalRecipients:_total,queuedCount:_queued,sentCount:_sent,deliveredCount:_delivered,failedCount:_failed,readCount:_read,...payload}=original;const item=await store.createCampaign({...payload,name:`${original.name} Copy`,status:'draft',recipientFilter:original.recipientFilter,createdBy:req.user!.id});res.status(201).json({campaign:item});});
  app.delete('/api/v1/admin/campaigns/:id', requirePermission(store, 'notifications:send'), async (req,res)=>{const campaign=await store.updateCampaign(String(req.params.id),{status:'archived'});assert(campaign,404,'CAMPAIGN_NOT_FOUND','Campaign not found');res.json({campaign});});
  app.delete('/api/v1/admin/campaigns/:id/permanent', requirePermission(store, 'notifications:send'), async (req,res)=>{assertPrivileged(req);const current=await store.findCampaign(String(req.params.id));assert(current?.status==='archived',409,'CAMPAIGN_NOT_ARCHIVED','Archive campaign before permanent deletion');const deleted=await store.deleteCampaign(current.id);assert(deleted,404,'CAMPAIGN_NOT_FOUND','Campaign not found');if(current.imageMediaId && (await store.mediaReferenceCount(current.imageMediaId))===0){const asset=await store.findMediaAsset(current.imageMediaId);if(asset){await store.updateMediaAsset(asset.id,{status:'archived'});await media.delete(asset.publicId).catch(()=>undefined);}}await store.audit('admin.campaign_deleted',{...clientMeta(req),userId:req.user!.id,metadata:{campaignId:current.id}});res.status(204).send();});
  app.get('/api/v1/admin/campaigns/:id', requirePermission(store, 'notifications:send'), async (req,res)=>{const campaign=await store.findCampaign(String(req.params.id));assert(campaign,404,'CAMPAIGN_NOT_FOUND','Campaign not found');const recipients=await store.listCampaignRecipients(campaign.id,{page:Number(req.query.page)||1,pageSize:Number(req.query.pageSize)||50,status:req.query.recipientStatus?String(req.query.recipientStatus):undefined});res.json({campaign,recipients});});
  app.patch('/api/v1/admin/campaigns/:id', requirePermission(store, 'notifications:send'), async (req,res)=>{const input=toInput(campaignPatchSchema,req.body);assert(!input.startDate || !input.endDate || new Date(input.startDate) <= new Date(input.endDate),400,'INVALID_CAMPAIGN_DATES','Campaign end date must be after its start date');const previous=await store.findCampaign(String(req.params.id));const campaign=await store.updateCampaign(String(req.params.id),{...input,emailHtml:sanitizeEmailHtml(input.emailHtml)});assert(campaign,404,'CAMPAIGN_NOT_FOUND','Campaign not found');if(previous?.imageMediaId && input.imageMediaId && previous.imageMediaId!==input.imageMediaId && (await store.mediaReferenceCount(previous.imageMediaId))===0){const asset=await store.findMediaAsset(previous.imageMediaId);if(asset){await store.updateMediaAsset(asset.id,{status:'archived'});await media.delete(asset.publicId).catch(()=>undefined);}}await store.audit('admin.campaign_updated',{...clientMeta(req),userId:req.user!.id,metadata:{campaignId:campaign.id,keys:Object.keys(input)}});res.json({campaign});});
  app.post('/api/v1/admin/campaigns/:id/send', requirePermission(store, 'notifications:send'), async (req,res)=>{const campaign=await store.findCampaign(String(req.params.id));assert(campaign,404,'CAMPAIGN_NOT_FOUND','Campaign not found');assert(!['archived','cancelled','completed'].includes(campaign.status),409,'CAMPAIGN_NOT_SENDABLE','This campaign cannot be queued in its current status');const queued=await store.queueCampaign(campaign.id,Array.isArray(req.body?.userIds)?req.body.userIds.map(String):undefined);assert(queued && queued.totalRecipients>0,400,'CAMPAIGN_NO_RECIPIENTS','No opted-in recipients match this campaign');await store.audit('admin.campaign_queued',{...clientMeta(req),userId:req.user!.id,metadata:{campaignId:campaign.id,recipients:queued.totalRecipients}});res.status(202).json({campaign:queued});});
  app.post('/api/v1/admin/campaigns/:id/retry', requirePermission(store, 'notifications:send'), async (req,res)=>{const campaign=await store.retryCampaignFailures(String(req.params.id));assert(campaign,404,'CAMPAIGN_NOT_FOUND','Campaign not found');await store.audit('admin.campaign_retry',{...clientMeta(req),userId:req.user!.id,metadata:{campaignId:campaign.id}});res.status(202).json({campaign});});
  app.post('/api/v1/admin/campaigns/:id/cancel', requirePermission(store, 'notifications:send'), async (req,res)=>{const campaign=await store.updateCampaign(String(req.params.id),{status:'cancelled'});assert(campaign,404,'CAMPAIGN_NOT_FOUND','Campaign not found');await store.audit('admin.campaign_cancelled',{...clientMeta(req),userId:req.user!.id,metadata:{campaignId:campaign.id}});res.json({campaign});});
  app.post('/api/v1/admin/campaigns/test', requirePermission(store, 'notifications:send'), async (req,res)=>{const input=toInput(campaignTestSchema,req.body);const destination=input.destination || req.user!.email || req.user!.phone;assert(destination,400,'TEST_DESTINATION_REQUIRED','Provide a test destination or set an admin email/phone');if(input.channel==='email')await messaging.sendEmail(destination,'Sadik Travels campaign test','This is a Sadik Travels campaign test.');else if(input.channel==='sms')await messaging.sendSms(destination,'Sadik Travels campaign test');else await store.createNotification({userId:req.user!.id,title:'Campaign test',message:'This is a Sadik Travels website notification test.',channels:['in_app'],status:'sent',createdBy:req.user!.id});res.json({sent:true,channel:input.channel});});
  app.post('/api/v1/admin/campaigns/templates', requirePermission(store, 'notifications:send'), async (req,res)=>{const input=toInput(templateInputSchema,req.body);const item=await store.createCampaignTemplate({...input,content:input.type==='email'?sanitizeEmailHtml(input.content)||'':input.content,createdBy:req.user!.id});res.status(201).json({template:item});});
  app.post('/api/v1/admin/campaigns/templates/:id/duplicate', requirePermission(store, 'notifications:send'), async (req,res)=>{const original=(await store.listCampaignTemplates()).find(item=>item.id===String(req.params.id));assert(original,404,'TEMPLATE_NOT_FOUND','Template not found');const item=await store.createCampaignTemplate({name:`${original.name} Copy`,type:original.type,subject:original.subject,content:original.content,metadata:original.metadata,status:'active',createdBy:req.user!.id});res.status(201).json({template:item});});
  app.delete('/api/v1/admin/campaigns/templates/:id', requirePermission(store, 'notifications:send'), async (req,res)=>{const item=await store.updateCampaignTemplate(String(req.params.id),{status:'archived'});assert(item,404,'TEMPLATE_NOT_FOUND','Template not found');res.json({template:item});});
  app.patch('/api/v1/admin/campaigns/templates/:id', requirePermission(store, 'notifications:send'), async (req,res)=>{const input=toInput(templateInputSchema.partial(),req.body);const item=await store.updateCampaignTemplate(String(req.params.id),{...input,content:input.content?sanitizeEmailHtml(input.content):input.content});assert(item,404,'TEMPLATE_NOT_FOUND','Template not found');res.json({template:item});});
  ...
  app.post('/api/v1/admin/content/:id/publish', requirePermission(store, 'content:manage'), async (req,res)=>{ assertPrivileged(req); const content=await store.updateContent(String(req.params.id),{status:'published'}); assert(content,404,'CONTENT_NOT_FOUND','Content item not found'); await store.audit('admin.content_published',{...clientMeta(req),userId:req.user!.id,metadata:{contentId:content.id}}); res.json({content}); });
  app.post('/api/v1/admin/content/:id/unpublish', requirePermission(store, 'content:manage'), async (req,res)=>{ assertPrivileged(req); const content=await store.updateContent(String(req.params.id),{status:'draft'}); assert(content,404,'CONTENT_NOT_FOUND','Content item not found'); await store.audit('admin.content_unpublished',{...clientMeta(req),userId:req.user!.id,metadata:{contentId:content.id}}); res.json({content}); });
  app.post('/api/v1/admin/content/:id/restore', requirePermission(store, 'content:manage'), async (req,res)=>{ assertPrivileged(req); const content=await store.updateContent(String(req.params.id),{status:'draft'}); assert(content,404,'CONTENT_NOT_FOUND','Content item not found'); await store.audit('admin.content_restored',{...clientMeta(req),userId:req.user!.id,metadata:{contentId:content.id}}); res.json({content}); });
  app.delete('/api/v1/admin/content/:id/permanent', requirePermission(store, 'content:manage'), async (req,res)=>{ assertPrivileged(req); const current=await store.findContent(String(req.params.id)); assert(current?.status==='archived',409,'CONTENT_NOT_ARCHIVED','Archive content before permanent deletion'); const deleted=await store.deleteContent(String(req.params.id)); assert(deleted,404,'CONTENT_NOT_FOUND','Content item not found'); await store.audit('admin.content_deleted',{...clientMeta(req),userId:req.user!.id,metadata:{contentId:String(req.params.id)}}); res.status(204).send(); });

  app.get('/api/v1/admin/media', requirePermission(store, 'content:manage'), async (req, res) => res.json(await store.listMediaAssets({ q: req.query.q ? String(req.query.q) : undefined, folder: req.query.folder ? String(req.query.folder) : undefined, status: req.query.status === 'active' || req.query.status === 'archived' || req.query.status === 'failed' ? req.query.status : 'all', page: Number(req.query.page) || 1, pageSize: Number(req.query.pageSize) || 24 })));
  app.post('/api/v1/admin/media', requirePermission(store, 'content:manage'), rateLimit('media-upload', 20, 60), mediaUpload.single('file'), async (req, res) => { assert(req.file, 400, 'IMAGE_REQUIRED', 'Choose an image to upload'); const folder = toInput(mediaFolderSchema, String(req.body.folder || 'general')); const altText = typeof req.body.altText === 'string' ? req.body.altText : undefined; const uploaded = await media.upload(req.file.buffer, { folder, originalFilename: req.file.originalname, declaredMime: req.file.mimetype, altText }); let asset; try { asset = await store.createMediaAsset({ ...uploaded, status: 'active', uploadedBy: req.user!.id }); } catch (error) { await media.delete(uploaded.publicId).catch(() => undefined); throw error; } await store.audit('admin.media_uploaded', { ...clientMeta(req), userId: req.user!.id, metadata: { mediaId: asset.id, publicId: asset.publicId, folder: asset.folder } }); res.status(201).json({ media: asset }); });
  app.post('/api/v1/admin/media/batch', requirePermission(store, 'content:manage'), rateLimit('media-batch-upload', 10, 60), mediaUpload.array('files', 10), async (req, res) => { const files = Array.isArray(req.files) ? req.files : []; assert(files.length > 0, 400, 'IMAGE_REQUIRED', 'Choose one or more images to upload'); const folder = toInput(mediaFolderSchema, String(req.body.folder || 'general')); const assets = []; for (const file of files) { const uploaded = await media.upload(file.buffer, { folder, originalFilename: file.originalname, declaredMime: file.mimetype, altText: typeof req.body.altText === 'string' ? req.body.altText : undefined }); try { assets.push(await store.createMediaAsset({ ...uploaded, status: 'active', uploadedBy: req.user!.id })); } catch (error) { await media.delete(uploaded.publicId).catch(() => undefined); throw error; } } await store.audit('admin.media_batch_uploaded', { ...clientMeta(req), userId: req.user!.id, metadata: { count: assets.length, folder } }); res.status(201).json({ media: assets }); });
  app.patch('/api/v1/admin/media/:id', requirePermission(store, 'content:manage'), async (req, res) => { const input = toInput(mediaPatchSchema, req.body); const asset = await store.updateMediaAsset(String(req.params.id), input); assert(asset, 404, 'MEDIA_NOT_FOUND', 'Media asset not found'); await store.audit('admin.media_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { mediaId: asset.id, status: input.status } }); res.json({ media: asset }); });
  app.post('/api/v1/admin/media/:id/replace', requirePermission(store, 'content:manage'), rateLimit('media-replace', 20, 60), mediaUpload.single('file'), async (req, res) => { assert(req.file, 400, 'IMAGE_REQUIRED', 'Choose an image to upload'); const current = await store.findMediaAsset(String(req.params.id)); assert(current, 404, 'MEDIA_NOT_FOUND', 'Media asset not found'); const uploaded = await media.upload(req.file.buffer, { folder: current.folder.split('/').pop() || 'general', originalFilename: req.file.originalname, declaredMime: req.file.mimetype, altText: typeof req.body.altText === 'string' ? req.body.altText : current.altText }); let asset; try { asset = await store.updateMediaAsset(current.id, { ...uploaded, status: 'active' }); } catch (error) { await media.delete(uploaded.publicId).catch(() => undefined); throw error; } assert(asset, 500, 'MEDIA_UPDATE_FAILED', 'Image replacement failed'); await media.delete(current.publicId).catch(() => undefined); await store.audit('admin.media_replaced', { ...clientMeta(req), userId: req.user!.id, metadata: { mediaId: asset.id, previousPublicId: current.publicId, publicId: asset.publicId } }); res.json({ media: asset }); });
  app.delete('/api/v1/admin/media/:id', requirePermission(store, 'content:manage'), async (req, res) => { const current = await store.findMediaAsset(String(req.params.id)); assert(current, 404, 'MEDIA_NOT_FOUND', 'Media asset not found'); assert((await store.mediaReferenceCount(current.id)) === 0, 409, 'MEDIA_IN_USE', 'This image is still referenced by published content or a tour'); await store.updateMediaAsset(current.id, { status: 'archived' }); try { await media.delete(current.publicId); } catch { throw new AppError(502, 'MEDIA_DELETE_FAILED', 'The asset was archived but could not be removed from Cloudinary'); } await store.audit('admin.media_archived', { ...clientMeta(req), userId: req.user!.id, metadata: { mediaId: current.id, publicId: current.publicId } }); res.json({ media: await store.findMediaAsset(current.id) }); });
