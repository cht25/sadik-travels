...
export function requireAdmin(store: Store): RequestHandler {
  return async (req: Request, _res: Response, next: NextFunction) => { try { await authenticate(store, req, true); next(); } catch (error) { next(error); } };
}

export function requirePermission(store: Store, permission: AdminPermission): RequestHandler {
  return async (req: Request, _res: Response, next: NextFunction) => { try { const user = await authenticate(store, req, true); if (!hasPermission(user, permission)) throw new AppError(403, 'PERMISSION_DENIED', `Permission required: ${permission}`); next(); } catch (error) { next(error); } };
}
...
export function requireFinePermission(store: Store, key: string): RequestHandler {
  return async (req: Request, _res: Response, next: NextFunction) => { try { const user = await authenticate(store, req, true); if (!hasFinePermission(user, key)) throw new AppError(403, 'PERMISSION_DENIED', `Permission required: ${key}`); next(); } catch (error) { next(error); } };
}
...
export function requireSuperAdmin(store: Store): RequestHandler {
  return async (req: Request, _res: Response, next: NextFunction) => { try { const user = await authenticate(store, req, true); if (user.role !== 'super_admin') throw new AppError(403, 'SUPER_ADMIN_REQUIRED', 'Only a Super Admin can perform this action'); next(); } catch (error) { next(error); } };
}

export function requireAuth(store: Store): RequestHandler {
  return async (req: Request, _res: Response, next: NextFunction) => { try { await authenticate(store, req, false); next(); } catch (error) { next(error); } };
}
